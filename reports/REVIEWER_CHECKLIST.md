# Reviewer checklist - điền khi kiểm bài người khác

Người gán: Bạn Cùng Nhóm   Người kiểm: Nguyen Don Quoc Tuan   Ngày: 2026-09-16

Chạy trước khi soi bằng mắt:

```bash
python3 tools/check_pose_labels.py --images dataset/images/train --labels <bài của họ>
python3 tools/visualize_pose.py --images dataset/images/train --labels <bài của họ> --out /tmp/vis_review
python3 tools/visibility_report.py --labels dataset/labels/train --compare <bài của họ>
```

| | Mục kiểm | Đạt? | Ghi chú / ảnh nào |
| --- | --- | --- | --- |
| 1 | Mọi người trong ảnh đều có đủ 17 điểm, không ai bị thiếu | [x] | |
| 2 | Bật đường nối: không có xương nào cắt chéo ở vai hoặc hông | [x] | |
| 3 | Không có xương nào kéo dài sang một cơ thể khác | [x] | |
| 4 | Khớp bị che dùng `v = 1` **và có chấm**, không phải `v = 0` | [x] | Một vài lỗi đã sửa. |
| 5 | `v = 0` chỉ xuất hiện ở khớp thật sự ra ngoài mép ảnh | [x] | |
| 6 | Không có dấu hiệu dùng `Hidden` (điểm `v = 2` nằm ở chỗ vô lý) | [x] | |
| 7 | Export đúng **COCO Keypoints 1.0**: mảng `keypoints` có 51 số mỗi người | [x] | |
| 8 | Bản YOLO Pose: mỗi dòng 56 số, `kpt_shape: [17, 3]` | [x] | |
| 9 | Visibility report đã nộp, và hai bảng đã được đặt cạnh nhau | [x] | |
| 10 | Mọi ca không rõ đều được ghi trong `GUIDELINE_MINI.md` | [x] | |
| 11 | `check_pose_labels.py` chạy 0 lỗi | [x] | Đã chạy lại không còn lỗi. |

## Lỗi tìm được

Chép sang `reports/review_partner.md`. Mỗi dòng một lỗi, đủ bốn cột - người sửa phải
mở đúng chỗ đó được mà không cần hỏi lại.

| Ảnh | Người thứ | Khớp | Lỗi gì | Sửa thế nào |
| --- | ---: | --- | --- | --- |
| 000000000285.jpg | 1 | right_knee | Đánh v=0 trong khi khớp bị che bởi bàn | Đổi thành v=1 và đặt điểm ước lượng |
| 000000000632.jpg | 2 | left_elbow | Đánh nhầm sang cùi chỏ người bên cạnh | Kéo lại điểm về tay người thứ 2 |
| 000000000139.jpg | 1 | right_ankle | Ra ngoài ảnh nhưng đánh v=1 | Đổi thành v=0 và xoá tọa độ |

## Hai câu kết luận

- Lỗi lặp đi lặp lại nhiều nhất của bài này: Nhầm lẫn giữa v=0 (ra ngoài mép ảnh) và v=1 (bị che khuất).
- Nó là lỗi **thao tác** hay lỗi **guideline chưa rõ**? Thao tác lúc mới làm chưa quen, sau khi nhắc đã sửa.
